  ...
  return (
    <div className="w-[280px] border-l border-gray-100 bg-white flex-shrink-0 flex flex-col overflow-y-auto">
      ...
      <div className="flex justify-center py-3">
        <div className={`w-[76px] h-[76px] rounded-full flex items-center justify-center text-2xl font-bold border-4 border-green-100 ${avColor(r)}`}>
          {initials(r)}
        </div>
      </div>
      ...
      <div className="flex justify-center gap-2 mt-3 pb-3 px-4">
        {[
          ...
          <button
            ...
            onClick={fn}
            title={label}
            className="w-9 h-9 rounded-full border border-gray-200 bg-white flex items-center justify-center text-gray-400 hover:bg-green-50 hover:border-green-400 hover:text-green-600 transition-all"
          >
            <Icon size={14} />
          </button>
        ))}
        ...
        <div className="flex flex-wrap gap-1">
          {(r.disability_type ?? []).length > 0
            ? r.disability_type!.map(d => (
                <span key={d} className="text-[11px] font-semibold bg-green-100 text-green-700 px-2 py-0.5 rounded-md">
                  {d}
                </span>
              ))
      ...
      <Section title="Employment">
        <div className="grid grid-cols-2 gap-x-3 gap-y-2.5">
          <Field label="Status"     value={r.employment_status} />
          <Field label="Income"     value={r.monthly_income ? `₱${r.monthly_income.toLocaleString()}` : undefined} />
      ...
      <div className="px-4 pb-5 mt-1">
        <button
          onClick={() => onView(r)}
          className="w-full bg-green-500 hover:bg-green-600 text-white text-sm font-bold py-2 rounded-lg transition-colors flex items-center justify-center gap-1.5"
        >
